...
public class JsonFormatEscapeTest {
  ...
  // Field-specific length boundaries

  // Name accepts up to 32 bytes (MAX_ASSET_NAME_LEN), while abbr accepts up to 5 bytes
  // (MAX_TOKEN_ABBR_NAME_LEN), so each field needs a payload that fits its own boundary.
  ...
  // Inbound compatibility and transaction ID stability
  ...
  private static void assertLegacyInboundMapping(String rawJsonToken) throws Exception {
    ...
    AssetIssueContract expectedAsset = AssetIssueContract.newBuilder()
        .setDescription(ByteString.copyFromUtf8(rawJsonToken))
        .build();
    Transaction actual = transactionWithAsset(actualAsset);
    Transaction expected = transactionWithAsset(expectedAsset);

    assertEquals("visible=true name-string bytes must retain the legacy raw-token mapping",
        expectedAsset.toByteString(), actualAsset.toByteString());
    assertEquals("the same create request must keep raw_data byte-identical across the upgrade",
        expected.getRawData().toByteString(), actual.getRawData().toByteString());
    assertEquals("the same create request must keep its txID across the upgrade",
        new TransactionCapsule(expected).getTransactionId(),
        new TransactionCapsule(actual).getTransactionId());
  }

  @Test
  public void testInboundNameStringMappingAndCreatedTxIdRemainCompatible() throws Exception {
    for (String rawJsonToken : new String[] {
        "plain",
        "C:\\\\dir",
        "a\\nb",
        "a\\\"b",
        "https:\\/\\/tron.network",
        "a\\u0041"}) {
      assertLegacyInboundMapping(rawJsonToken);
    }
  }
  ...
  // U+FFFF sentinel regression

  // CharacterIterator.DONE is the real code point U+FFFF, so an iterator-driven escape loop
  // silently dropped everything from the first U+FFFF onward.
  ...
  @Test
  public void testProtoStringInboundMappingRemainsCompatible() throws Exception {
    for (boolean visible : new boolean[] {true, false}) {
      final boolean currentVisible = visible;
      org.junit.Assert.assertThrows(
          "escaped solidus must retain the develop-branch rejection behavior",
          JsonFormat.ParseException.class, () -> mergeId("a\\/b", currentVisible));
  ...
  // JsonGenerator newline handling
  ...
  // Name-string bytes are screened by isValidUtf8(), but proto string fields reach escapeText()
  // directly. An unpaired surrogate there used to let IllegalArgumentException escape from
  // printToString. It is now rendered as '?', matching Java and protobuf UTF-8 substitution,
  // so output does not depend on whether the message went through a protobuf round trip.
  ...
  // P4: serialization does not modify the source protobuf bytes. Inbound reconstruction is
  //     deliberately outside this outbound-only security fix because changing it changes txIDs.
